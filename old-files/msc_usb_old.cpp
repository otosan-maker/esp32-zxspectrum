...
static esp_err_t storage_init_spiflash(wl_handle_t *wl_handle)
{
    ...
    if (data_partition == NULL) {
        ...
        Serial.println("Failed to find FATFS partition. Check the partition table.");
...
void setupStorageShare(){
    //esp_log_level_set(TAG,CONFIG_LOG_DEFAULT_LEVEL_VERBOSE);
    ESP_LOGI(TAG,"Setup USB share");
    Serial.println("Setup USB share");
    static wl_handle_t wl_handle = WL_INVALID_HANDLE;
    ESP_ERROR_CHECK(storage_init_spiflash(&wl_handle));

    const esp_vfs_fat_mount_config_t i_mount_config ={
        .max_files = 5,
    };
    const tinyusb_msc_spiflash_config_t config_spi = {
        .wl_handle = wl_handle,
        .callback_mount_changed = storage_mount_changed_cb, 
        .mount_config = i_mount_config,
    };
    ESP_LOGI(TAG, "vamos a llamar a ... tinyusb_msc_storage_init_spiflash");
