...
static esp_err_t _mount(char *drv, FATFS *fs)
{
    ...
    if (fresult != FR_OK) {
        ...
        //fresult = f_mkfs("", &opt,(DWORD)0, workbuf, workbuf_size); // Use default volume
        if (fresult != FR_OK) {
            ret = ESP_FAIL;
...
{
    ESP_LOGI(TAG, "enter tinyusb_msc_storage_init_spiflash");
    assert(!s_storage_handle);
    ...
    ESP_LOGI(TAG, "variable config tinyusb_msc_storage_init_spiflash");
